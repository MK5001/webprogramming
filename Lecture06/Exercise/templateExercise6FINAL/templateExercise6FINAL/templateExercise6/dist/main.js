    ...
    startButton.addEventListener("click", () => {
        ...
        function nextQuestion() {
            if (currentQuestionIndex < selectedQuestions.length) {
                ...
                renderQuestion(selectedQuestions[currentQuestionIndex], (answer) => {
                    ...
                    if (leaderboardList) {
                        ...
                        players.forEach(player => {
                            ...
                            const maxPoints = selectedQuestions.reduce((acc, question) => {
                                const points = { easy: 1, medium: 2, hard: 3 };
                                return acc + points[question.difficulty];
                            }, 0);
                            const playerPercentage = (player.score / maxPoints) * 100;
                            leaderboardList.innerHTML += `<li>${player.name}: ${player.score} Punkte (${playerPercentage.toFixed(2)}%)</li>`;
                        });
                    }
                    // Nächste Frage
                ...
                quizContainer === null || quizContainer === void 0 ? void 0 : quizContainer.classList.remove("show"); // Overlay ausblenden
                ...
                if (refreshGameBtn) {
                    refreshGameBtn.style.display = "inline-block";
