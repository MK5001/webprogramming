// Global variables
let currentInterval: number | null = null; // Variable, um das aktuelle Intervall zu speichern
let SongIsPlaying: boolean = false; // Variable, um zu überprüfen, ob ein Song abgespielt wird
...
document.querySelectorAll('.key').forEach((key: Element) => {
    key.addEventListener('click', () => {
        const note = keys.find(k => k.id === key.id)?.note;  // ? bedeutet, dass es nur ausgeführt wird, wenn es einen Wert gibt
        if(note){
...
// fetched die songs aus der songs.json und spielt sie bei auslösen des Eventlisteners ab
async function LoadJson() {  // async function braucht ein Promise, weil es auf die Antwort des Servers wartet
...
function displayNotes(song: string[]): void {
...
function toggleStopButton(show: boolean): void {
    const stopButton = document.getElementById('stop');
...
function toggleResetButton(show: boolean): void {
